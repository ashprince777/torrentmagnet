...
async function getTrendingFromTpb() {
  const res = await fetch('https://thepiratebay10.org/top/all', {
    headers: {
      'User-Agent':
  ...
  if (!items || items.length === 0) throw new Error('No items parsed from TPB trending');
  return items;
}

async function getTrendingFromApibay() {
  const res = await fetch('https://apibay.org/precompiled/data_top100_all.json', {
